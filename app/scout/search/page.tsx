...
import { Search, Filter, Star, Zap, Target, TrendingUp, Shield, ChevronRight } from 'lucide-react'
...
export default function DiscoveryDashboard() {
  ...
  const filteredAthletes = athletes.filter(a => {
    const matchesSearch = a.full_name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          a.current_club.toLowerCase().includes(searchTerm.toLowerCase())
    const matchesPosition = filterPosition === 'All' || a.position === filterPosition
    return matchesSearch && matchesPosition
  })

  return (
    <div className="min-h-screen bg-[#0D0D1A] text-white pt-24 pb-20">
                  ...
                  <div className="p-8 -mt-10 relative z-10">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-6">
                      ...
                      <div className="p-3 bg-white/5 rounded-2xl border border-white/5">
                        ...
                        <div className="text-xs font-bold">{new Date().getFullYear() - new Date(athlete.birth_date).getFullYear()}</div>
                      </div>
                    </div>
